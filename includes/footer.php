

<style>
    .text-custom-blue {
        color: #6699cc;
    }
</style>


<footer class="bg-black">    
    <div class="container mx-auto py-4 row">
        <!-- Coluna do Logo -->
        <div class="col-md-4 text-center">
            <img alt="Groove it up logo" src="imagens/logo_branco.png" style="width: 190px;">
        </div>

        <!-- Coluna de Informações -->
        <div class="col-md-4 text-center mt-3">                                              
            <h5 class="text-custom-blue font-weight-bold mb-3">Informações</h5>
                <ul class="list-unstyled text-white">
                    <li>Termos e Condições</li>
                    <li>Política de Privacidade</li>
                    <li>Política de Cookies</li>
                </ul>
        </div>

        <!-- Coluna de Contactos -->
        <div class="col-md-4 text-center mt-5">
            <ul class="list-unstyled text-white">
                <li class="align-items-center">
                    <i class="fas fa-envelope text-custom-blue mr-2"></i>
                    <span>geral@groove_station.pt</span>
                </li>
                <li class="align-items-center mt-2">
                    <i class="fas fa-phone text-custom-blue mr-2"></i>
                    <span>(+351) 999 999 999</span>
                </li>
            </ul>
        </div>
                    
                
        <div class="text-center text-white mt-5">
            <p>© 2024 José Menezes | João Luís  </p>
            <p>Groove Station you, because you rock!</p>
        </div>
    </div>        
</footer>



